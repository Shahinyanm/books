<template>
    <form class="col-md-3" v-on:submit.prevent="handleSubmit">
        <div class="alert alert-success" role="alert" v-if="updated">
            <strong>Success!</strong> Author is updated
        </div>
...
<script>
    export default {
        ...
        mounted() {
            this.$http.get('author/' + this.id).then((response) => {
                this.form = response.data.data.author;
            }).catch((error) => {
        ...
        methods: {
            handleSubmit: function (e) {
                ...
                this.updated = false;
                this.errors = {};
                this.$http.post('author/' + this.id, this.form).then(() => {
                    this.updated = true;
                }).catch((error) => {
                    this.errors = error.response.data.errors.validation;
                });
